...
- **OpenMeet is frozen and must be hidden, not extended** (#186). Do not add OpenMeet features, dependencies, OAuth scopes, UI, MCP tools, or new call paths. Existing routes/helpers and `openmeetEventSlug` remain temporarily only as dormant compatibility code for old records; preserve read compatibility until a separately approved deletion pass.
...
- `OPENMEET_TENANT_ID` — legacy/dormant OpenMeet compatibility setting (default: `lsdfaopkljdfs`); do not use for new work (#186)
