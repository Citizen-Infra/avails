...
- **The .ics UID namespace is frozen and must never track the host.** `icsUidFor()` ends `@avails.citizeninfra.org`, and that is a namespace, not a deployment address. A `METHOD:CANCEL` only removes an event from a calendar when its UID matches the REQUEST already imported (RFC 5545 §3.8.4.7), so changing the right-hand side orphans every outstanding invite — silently, since nothing errors. It *was* derived from `CLIENT_URL` until 2026-08-04, so the `avails.zhgnv.com` move did exactly that; it was frozen at that moment because every event invited under the old host had already happened, making the exposure zero. **Residual gap:** an invite issued before 2026-08-04 carries `@avails.zhgnv.com` and cannot be cancelled by UID. All such events are in the past, so this is closed in practice rather than outstanding.
...
Fields:
- `scope: { type: 'atproto-list' | 'ca-community' | 'ca-event', value }` — the single group or event this offer is for. `atproto-list` takes an `at://…/app.bsky.graph.list/…` URI; `ca-community` takes a community-admin community id; `ca-event` takes the stable event DID. Scope semantics — normalize, validate, compare — live in one module, `server/src/mcp/scope.js`, shared by writers, resolvers, and tools. List and community writes retain their existing behavior. Every `ca-event` create or replacement calls Community Admin's event-participant introspection online using the authenticated participant DID and `publish-standing-availability`; inactive decisions return 403, while producer outages or missing consumer configuration return 503 without a write. Avails never falls back to a Bluesky graph or list query for event authorization.
...
One record per `(scope.type, scope.value)` — publishing again for the same scope replaces the deterministic-key record rather than creating a duplicate.
...
`ca-event` inspection and deletion remain available after grant loss. Only a new or replacement write is gated. Event records remain public participant-owned PDS records; grant loss does not delete them, and expired records are filtered from overlap reads.
...
- **`schedule_call` MCP tool** (`server/src/mcp/tools.js`) — no-auth-required booking flow: resolve `scope` → `resolveListAvailability` → `bestCallSlots` → coverage check → trust split → book. Coverage floor (`MIN_CALL_COVERAGE = 2`): fewer than 2 members with a record, or a top slot with fewer than 2 people free, returns `{ booked: false, fallback: 'create_poll', reason }` instead of booking — it signals the fallback, it does not create the poll itself. On a bookable slot, members with `trust: 'auto'` are auto-booked; everyone else (including any unrecognized or missing trust value) lands in `needsConfirm`, never silently committed. Reuses `generateIcs` (object-form, like every other call site) and best-effort `sendEmail` to any member record carrying an email (rare — email isn't a lexicon field). **No poll record is created anywhere on the success path** — that's the Phase-1 done criterion.

**Retrying `schedule_call` is safe only with an `idempotencyKey` (#166).** The tool books a real call and mails real people, and a request that succeeds here but whose response is lost — a timeout at 15s while the booking completed at 16s, a dropped connection, a container replaced mid-reply — is indistinguishable, from the caller's side, from one that never arrived. community-admin's `fire()` deliberately retries that case rather than recording it as "avails declined", which is right (a network failure recorded as a refusal would tell a community nobody could meet when nobody was asked) and which is exactly why no caller-side lock can fix it: the caller genuinely does not know. Only the callee can answer, by remembering. So a caller supplies a stable key (CA derives it from the proposal id), avails records the booking under it in `server/src/mcp/bookingLedger.js`, and a repeat returns that booking with `alreadyBooked: true` — a third answer, distinct from both "avails declined" and "we could not ask". Three details are load-bearing: the key is **caller-supplied, not derived from the request's shape**, because two genuinely different calls for the same group in the same window are legitimate and a natural key would silently collapse them; only **successful** bookings are remembered, since a `booked: false` coverage answer is a legitimate "ask again later"; and the ledger is written **before the invitations go out and awaited, not left to the 30-second flush**, so the one sliver no ledger closes (a crash between booking and recording) falls on the side of a missed invitation rather than a duplicate one. Without a key the old behaviour stands — a lost response books again — which is fine for a person asking once and not for anything automated. An in-process claim also refuses two overlapping calls on the same key; that guard stops at the process boundary, the durable ledger does not.

**`ca-community` scope has no whole-group path and cannot have one.** avails can enumerate a Bluesky list from the AppView; a community's roster is not in ATProto at all. So `schedule_call` requires `voterDids` for a `ca-community` scope and fails naming that argument rather than resolving zero members — which would be reported as "nobody has published availability" and send an organizer chasing a coverage problem that does not exist. It also requires the service credential, because there is no owner DID in a community id to compare a caller against; without that branch the list-owner check happens to reject too, but with a message about a list that isn't one. community-admin's call-proposal trigger already supplies the DIDs, since it counts the likers of the proposal post. Whole-community scheduling would need CA's `GET /api/members` (community-admin#55), which is not wired here.

**`ca-event` is readiness-only.** The service-authenticated `evaluate_availability_overlap` MCP operation accepts an exact event DID, eligible supporter DIDs, date window, duration, and threshold. It reads only matching, unexpired participant-owned records and returns aggregate record coverage, maximum shared overlap, and the top candidate slot. It imports no booking, email, ICS, poll, or list-enumeration code. `schedule_call` rejects `ca-event` explicitly because an event-participant grant does not authorize booking or invitations.
...
- ~~**Pre-existing, unrelated bug surfaced while building this feature — the MCP `schedule` tool's finalize path calls `generateIcs` positionally.**~~ **FIXED** (#105, `test/mcp-schedule-ics.test.js`). `generateIcs` (`server/src/lib/ical.js`) takes a single destructured options object; the MCP `schedule` tool passed it positionally, so `poll` destructured to `undefined` and the call threw on every finalize through that tool — after the record had already been written, leaving the poll finalized but the tool errored. All four call sites now use the object form. Left here rather than deleted because the shape of the failure is worth remembering: a positional call to a destructuring function fails at the first property read, not at the call, so it looked like bad data rather than a bad call.

## Meeting links (#19)

A finalized poll can carry a `meetingUrl` — a video call link, on the poll record beside `finalTime`. v1 is the manual half: paste any http(s) URL (Jitsi, Meet, Zoom). A Jitsi room needs no account and can simply be named, so `https://meet.jit.si/avails-<rkey>` works as a generated default. Zoom OAuth and Google Meet stay deferred; **Meet comes nearly free** once #7's server-side insert lands, as a `conferenceData` request on the same call.

**`normalizeMeetingUrl` (`server/src/lib/meetingUrl.js`) is a security boundary, not a formatter.** The value is user-supplied, stored in a world-readable record, rendered as an `href`, and written into an `.ics` — three consumers, each with its own way to be hurt by a hostile string, so the check lives in one place and every writer goes through it (the REST middleware and the MCP `schedule` tool both call it).

- **Scheme allowlist.** `javascript:` and `data:` parse perfectly well as URLs; without the allowlist each is a working XSS the moment the banner renders one. There is no sanitiser downstream.
- **CRLF rejected *before* parsing.** iCalendar is line-oriented and built on CRLF, so a newline in a property value can forge further properties. The order is load-bearing: WHATWG URL silently strips tabs and newlines, so checking the parsed `href` would launder a hostile string and report success.

**Three-way write semantics.** Omitted means "leave alone", a URL sets, an empty string clears (deleting the key rather than storing `''`). Collapsing omitted-and-cleared would let any client that predates the feature wipe the link on every finalize.

**In the `.ics`**, the link goes in `LOCATION` *and* the description. `LOCATION` is where Google, Outlook and Apple look for a join affordance; the description is what survives a client that renders none. `URL` stays the poll — that is the durable record of the decision, and it is what should still be useful after the meeting has passed. A `METHOD:CANCEL` carries no join link at all: a room on a meeting that is not happening invites someone to sit in it alone.
