...
Fields:
- `scope: { type: 'atproto-list' | 'ca-community', value }` — the single group this offer is for. **Both kinds are accepted.** `atproto-list` takes an `at://…/app.bsky.graph.list/…` URI; `ca-community` takes a community-admin community id. The two differ in who can *resolve* them, not in who can write one: avails reads a Bluesky list's membership from the public AppView, but a community's roster lives in community-admin, so a `ca-community` scope can only be scheduled by an authorized service supplying the DIDs (see `schedule_call` below). Scope semantics — normalize, validate, compare — live in one module, `server/src/mcp/scope.js`, shared by the resolver and the tool: a scope that normalized one way and matched another would book someone from a group they never offered themselves to. **Membership is deliberately not verified when writing**, for either kind: an offer scoped to a group you are not in is inert, since only that group's scheduler ever reads it, and the same has always been true of a Bluesky list, which anyone can name without being on it. Consent to be booked lives in `trust`.
